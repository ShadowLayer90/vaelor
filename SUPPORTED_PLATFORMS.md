...
| Ubuntu 26.04 AMD64 workstation (HP Z2 Mini G1a) | Compatible (hardware probed, package not yet accepted) | Absent with a reason | Full | CPU; AMD accelerator discovered and sized | Desktop edition only | Full |
...
Vaelor's own inference backends cannot target one, and offload through an
external runtime is **unverified** here. Note that `/dev/accel/*` is owned by
the same `render` group as the GPU nodes, so group membership cannot separate
NPU access from GPU access; only per-device passthrough can.
