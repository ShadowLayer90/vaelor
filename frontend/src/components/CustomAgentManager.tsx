...
export function CustomAgentManager({
  ...
  return (
    <section className="custom-agent-manager">
      <header>
        <div><span className="section-eyebrow">Agent workshop</span><h2>Build and manage custom agents</h2><p>Define a general-purpose agent for your own domain. Choose its model context and exact data or action grants; every edit creates a new version.</p></div>
      ...
      {draft && (
        ...
        <form className="custom-agent-form" onSubmit={(event) => void save(event)}>
         <div className="panel-heading"><div><span className="section-eyebrow">Guided agent setup</span><h3 id="custom-agent-editor-title">{draft.id ? "Edit custom agent" : "Create custom agent"}</h3><p>Define its job, then grant only the information and actions it needs. Nothing can run until you save this version.</p></div><Button variant="quiet" aria-label="Close agent editor" disabled={busy} onClick={() => setDraft(null)}>Close</Button></div>
          {/*
          ...
          <section className="custom-agent-review" aria-labelledby="custom-agent-review-heading">
            <div><span className="section-eyebrow">Step 5 · Review</span><h4 id="custom-agent-review-heading">Confirm intent and access before saving</h4><p>{draft.name || "This agent"} will be created as a versioned definition. The runner receives only the grants listed below.</p></div>
      ...
      {testAgent && (
        <div className="custom-agent-test-backdrop" role="presentation" onKeyDown={(event) => { if (event.key === "Escape") closeRun(); }}>
          {activeRun ? (
            <div aria-labelledby="custom-agent-test-heading" aria-modal="true" className="custom-agent-test-dialog" role="dialog">
            ...
            <form aria-labelledby="custom-agent-test-heading" aria-modal="true" className="custom-agent-test-dialog" onSubmit={(event) => void queueTestRun(event)} role="dialog">
              <div className="panel-heading"><div><span className="section-eyebrow">Approval-gated run</span><h3 id="custom-agent-test-heading">Run {testAgent.name}</h3><p>Describe one realistic task. Vaelor prepares a reviewable run and shows the result here; nothing executes until you approve it.</p></div><Button variant="quiet" aria-label="Close test run" onClick={closeRun} type="button">Close</Button></div>
