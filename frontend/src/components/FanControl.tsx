...
export function FanControl({
  ...
  return (
    <div className="fan-page">
      ...
      <div className="system-section-nav" role="tablist" aria-label="System sections">
        {systemTabs.map((tab, index) => (
          <Button
            ...
            className={activeSection === tab.id ? "system-section-nav__tab is-active" : "system-section-nav__tab"}
            ...
            onKeyDown={(event) => {
              ...
              const nextIndex = (index + (event.key === "ArrowRight" ? 1 : -1) + systemTabs.length) % systemTabs.length;
              navigateSection(systemTabs[nextIndex].id);
              document.getElementById(`system-tab-${systemTabs[nextIndex].id}`)?.focus();
            }}
            role="tab"
            type="button"
          >
      ...
      {activeSection === "cooling" && <div id="system-panel-cooling" role="tabpanel" aria-labelledby="system-tab-cooling" tabIndex={-1}>
      ...
      {activeSection === "compute" && <div id="system-panel-compute" role="tabpanel" aria-labelledby="system-tab-compute" tabIndex={-1}><ComputePanel machine={resolvedMachine} /></div>}
      {activeSection === "lighting" && <div id="system-panel-lighting" role="tabpanel" aria-labelledby="system-tab-lighting" tabIndex={-1}><LightingControl session={session} /></div>}
      {activeSection === "hardware" && <div id="system-panel-hardware" role="tabpanel" aria-labelledby="system-tab-hardware" tabIndex={-1}><SystemInventoryPanel session={session} /></div>}
