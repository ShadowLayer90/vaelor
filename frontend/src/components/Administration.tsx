...
export function Administration({ session, onBack }: { session: Session; onBack?: () => void }) {
  ...
  // Mirrors applyFactoryReset for the irreversible full uninstall.
  const applyRemoveVaelor = async () => {
    setBusy("remove-apply"); setMessage("");
    try {
      await apiRequest(
        "/admin/recovery/remove-vaelor",
        {
          method: "POST",
          body: JSON.stringify({ confirmation: removeConfirmation }),
        },
        session.csrf_token,
      );
      setMessage("Removal started. Every Vaelor service, its data, and the OS stack it installed are being removed; this session will end and the appliance will be gone.");
    } catch (error) {
      reportFailure(error,"Removal was not accepted.");
    } finally { setBusy(""); }
  };
    ...
    <div className="admin-page">
      <div className="page-heading">
        ...
        <div className="workspace-route-actions">
          ...
          <StatusPill label="Administrator" tone="success" />
        </div>
      </div>
      ...
      {tab === "users" && (
        <div aria-labelledby="settings-tab-users" className="admin-layout" id="settings-panel-users" role="tabpanel" tabIndex={-1}>
          ...
          <section className="data-panel admin-create mfa-panel" aria-labelledby="mfa-title">
            <div className="panel-heading"><div><h2 id="mfa-title">Your two-factor sign-in</h2><p>Add an authenticator code after your password.</p></div><StatusPill label={totpEnabled ? "Enabled" : "Optional"} tone={totpEnabled ? "success" : "neutral"} /></div>
              ...
              <StatusPill
                label={item.last_test_status === "success"
                  ...
                  : item.last_test_status === "failed"
                    ...
                    : "Not tested"}
                tone={item.last_test_status === "success" ? "success" : item.last_test_status === "failed" ? "warning" : "neutral"}
              />
            </div>
  ...
  />
          ...
          <div className="credential-list"><PaginatedItems items={apiTokens} label="External API connections" pageSize={8} render={(item) => <div className="credential-row" key={item.id}><span className="credential-row__icon"><Icon name="network" /></span><span><strong>{item.label}</strong><small>{item.scopes.map((scope) => scope === "assistant" ? "Assistant" : "Inference").join(" + ")} · {item.prefix}… · {item.last_used_at ? `used ${new Date(item.last_used_at * 1000).toLocaleString()}` : "never used"}</small></span><StatusPill label={item.enabled ? "Active" : "Revoked"} tone={item.enabled ? "success" : "neutral"} />{item.enabled && <Button variant="danger" disabled={busy === `api-${item.id}`} onClick={() => setRevokeToken(item)}>Revoke</Button>}</div>} /></div>
        </section>
        </div>
      )}
...
<Input autoComplete="off" id="import-confirmation" label={<span>Type <strong>{portableState.confirmation}</strong></span>} onChange={(event) => setImportConfirmation(event.target.value)} value={importConfirmation} />
        ...
        <section className="data-panel admin-wide factory-reset" aria-labelledby="factory-reset-title">
          <div className="panel-heading">
            ...
            <StatusPill label="Approval required" tone="neutral" />
          </div>
...
<Input autoComplete="off" id="reset-confirmation" label={<span>Type <strong>{recovery?.confirmation ?? "RESET VAELOR"}</strong> to unlock the final action</span>} onChange={(event) => setResetConfirmation(event.target.value)} value={resetConfirmation} />
        ...
        <section className="data-panel admin-wide factory-reset" aria-labelledby="remove-vaelor-title">
          <div className="panel-heading">
            <div>
              <span className="page-eyebrow">Irreversible uninstall</span>
              <h2 id="remove-vaelor-title">Remove Vaelor</h2>
              <p>This does not return Vaelor to first-run setup — it removes it entirely. Every Vaelor service and its data, plus the OS stack Vaelor installed (Docker, InfluxDB, the ROCm packages, amd-smi, and novnc), are deleted. This cannot be undone.</p>
            </div>
            <StatusPill label="Approval required" tone="neutral" />
          </div>
          ...
          <div className="factory-reset__scope">
            <section>
              <h3>Will be permanently removed</h3>
              <ul>{removeVaelor?.removes.map((item) => <li key={item}>{item}</li>)}</ul>
            </section>
            <section>
              <h3>Will remain</h3>
              <ul>{removeVaelor?.retains.map((item) => <li key={item}>{item}</li>)}</ul>
            </section>
          </div>
          ...
          <div className="factory-reset__confirmation">
              <div>
                <strong>Review complete</strong>
                <small>Nothing is removed until the final action is approved.</small>
              </div>
<Input autoComplete="off" id="remove-confirmation" label={<span>Type <strong>{removeVaelor?.confirmation ?? "REMOVE VAELOR"}</strong> to unlock the final action</span>} onChange={(event) => setRemoveConfirmation(event.target.value)} value={removeConfirmation} />
              <div className="factory-reset__actions">
                <Button variant="danger"
                  disabled={Boolean(busy) || !removeVaelor || removeConfirmation !== removeVaelor.confirmation}
                  onClick={() => void applyRemoveVaelor()}
                  type="button"
                >
                  {busy === "remove-apply" ? "Starting removal…" : "Approve and remove Vaelor entirely"}
                </Button>
              </div>
          </div>
        </section>
