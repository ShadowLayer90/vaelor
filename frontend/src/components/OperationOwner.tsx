...
export interface OperationOwnerProps {
  ...
  /** Called after "Done" acknowledges a finished operation, so the owner can
   *  dismiss its view of it. `controller.done` only clears the resume key; a
   *  parent that tracks the operation in its own state (not via that key) must
   *  clear it here or the closed operation stays on screen. */
  onDone?: () => void;
...
export function OperationOwner({
  ...
  // "Done" acknowledges a finished operation. `controller.done` only forgets
  // the resume key, so an owner that tracks the operation elsewhere is told to
  // dismiss it through `onDone` once that has run - without it the closed
  // operation stays on screen and the button looks inert.
  const controllerDone = controller?.done;
  const finishOperation = controllerDone
    ? async () => {
        const result = await controllerDone();
        onDone?.();
        return result;
      }
    : undefined;
    ...
    >
      ...
      {!readOnly && (
        <footer className="operation-owner__actions" aria-label="Operation actions">
          ...
          {actionButton("done", canDone, finishOperation, "primary")}
        </footer>
      )}
