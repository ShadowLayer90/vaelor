    ...
    <section className="integration-capabilities__card integration-capabilities__step" aria-labelledby="integration-connection-title">
      ...
      <div className="integration-capabilities__step-content">
        <div className="integration-capabilities__section-heading">
          ...
          {onCreate && <Button className="integration-capabilities__button" disabled={disabled} onClick={onCreate} type="button" variant="quiet">Create connection</Button>}
        </div>
...
function GrantPreview({
  ...
  return (
    <section className="integration-capabilities__preview" aria-labelledby="integration-preview-title">
      ...
      <div className="integration-capabilities__preview-footer"><small>Preview expires {preview.expiresAt}</small><Button className="integration-capabilities__button" disabled={saving} onClick={onSave} type="button" variant="primary">{saving ? "Saving grant…" : "Save grant"}</Button></div>
    </section>
  );
}
    ...
    <section className="integration-capabilities__card integration-capabilities__dependents" aria-labelledby="integration-dependents-title">
      ...
      <details className="integration-capabilities__details" open={showRevoke} onToggle={(event) => { if (event.currentTarget.open !== showRevoke) onToggleRevoke(); }}>
        ...
        <div className="integration-capabilities__revoke-confirmation">
          ...
          <Button className="integration-capabilities__button" disabled={revoking || grant.status === "revoked"} onClick={onRevoke} type="button" variant="danger">{revoking ? "Revoking…" : "Revoke grant"}</Button>
        </div>
      </details>
    </section>
  );
...
function ErrorState({ error, onRetry }: { error: string; onRetry: () => void }) {
  return <div className="integration-capabilities__state integration-capabilities__state--error" role="alert"><Icon name="alert" size={24} /><strong>Capabilities could not be loaded</strong><span>{error}</span><Button className="integration-capabilities__button" onClick={onRetry} type="button" variant="quiet">Retry</Button></div>;
}
