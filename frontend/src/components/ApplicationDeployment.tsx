...
export function ApplicationDeployment(props: ApplicationDeploymentProps) {
  ...
  useEffect(() => {
    ...
    approvalCloseRef.current?.focus();
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape" && asyncState !== "loading") setShowApproval(false);
      if (event.key !== "Tab" || !approvalRef.current) return;
      const controls = [...approvalRef.current.querySelectorAll<HTMLElement>("button:not(:disabled), [href], input:not(:disabled)")];
    ...
    <section className="application-deployment" aria-labelledby="application-deployment-title">
      ...
      {phase === "request" && <section className="application-deployment__panel" aria-labelledby="application-request-heading">
        ...
        <form onSubmit={classify} className="application-deployment__request-form">
          ...
          <Button type="submit" disabled={props.disabled || asyncState === "loading" || !request.trim()}>{asyncState === "loading" ? (phase === "request" ? "Understanding request…" : "Researching automatically…") : intent ? "Research again" : "Research and prepare plan"}</Button>
        </form>
      ...
      {intent && phase === "research" && <section className="application-deployment__panel" aria-labelledby="application-research-heading">
        ...
        {research?.status === "complete" && <>
          ...
          {phase === "research" && <Button type="button" onClick={() => holdPhase("configure")} disabled={research.compatibility === "unsupported" || !research.sources.length}>Configure deployment</Button>}
        </>}
      </section>}
      ...
      {research?.status === "complete" && phase !== "request" && phase !== "research" && configurable && <section className="application-deployment__panel" aria-labelledby="application-configure-heading">
        ...
        <form className="application-deployment__configuration" onSubmit={generateDraft}>
          ...
          <Button type="submit" disabled={props.disabled || asyncState === "loading" || requiredSecretsMissing || advancedConfigInvalid}>{asyncState === "loading" ? "Building draft…" : draft ? "Regenerate validated draft" : "Generate validated Compose draft"}</Button>
        </form>
      </section>}
