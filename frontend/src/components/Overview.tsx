...
import { hasNeuralAccelerator, machineNoun, thermalLimits, unknownMachine } from "../lib/machine";
        ...
        <main className="main" id="main-content" tabIndex={-1}>
          ...
          <section className="device-hero" aria-label="System summary">
            ...
            <figure className="device-hero__visual">
              ...
              {/*
                * An illustration is a claim about a machine, so it has to be a
                * claim that holds. Off-Pi the artwork used to fall through to
                * `Generic` — a plain box with three coloured dots — and the
                * objection was never to artwork, it was that a generic drawing
                * standing in for a specific machine is clip art of something
                * this is not. There is now a drawing of *this* class, so the
                * objection is answered where it arose.
                *
                * The neural mark follows discovery, never the machine class: a
                * workstation is not guaranteed an accelerator, and a drawing
                * that showed one anyway would promise an engine the rest of
                * the product then correctly refuses to report.
                */}
              {isAppliance
                ? <PironmanDeviceIcon
                  ...
                  size={260}
                />
                : <WorkstationDeviceIcon
                  label={`${device?.name ?? "This machine"} technical hardware illustration`}
                  neural={hasNeuralAccelerator(resolvedMachine)}
                  size={260}
                />}
              <figcaption>
