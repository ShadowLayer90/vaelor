...
export function Workloads({ session }: { session: Session }) {
  ...
  const queueModelDeploy = async (job: WorkloadJob) => {
    ...
    try {
      const deployment = await apiRequest<WorkloadJob>("/jobs", {
        method: "POST",
        body: JSON.stringify({ type: "model.deploy", payload: {
          path: job.result.path, port: 0, mode: modelRuntimeMode,
        } }),
      }, session.csrf_token);
  ...
  const composeReady = Boolean(capabilities?.docker.installed && capabilities?.docker.compose);
  ...
  return (
    <div className="workloads-page">
      <div className="page-heading">
        ...
        <StatusPill label={composeReady ? "Docker ready" : dockerInstallAvailable ? "Docker setup available" : loadError ? "Check interrupted" : "OS limits apply"} status={composeReady ? "healthy" : loadError ? "degraded" : "neutral"} />
      </div>
      ...
      <section className="workload-runtime" aria-label="App setup readiness">
        ...
        <div>
          ...
          {/*
            ...
            */}
          <strong>{!capabilities ? "Checking whether this node can run apps" : composeReady ? "This Vaelor node is ready to run apps" : dockerInstallAvailable ? `Docker is not installed on ${capabilities?.os?.name ?? "this host"}` : capabilities?.docker.installed ? "Docker Compose is not ready" : "Container apps are unavailable on this OS"}</strong>
          <span>{!capabilities ? "Nothing has been established yet, so nothing here is a verdict about this machine." : composeReady ? "No terminal or Linux commands required." : capabilities?.docker.installation?.reason ?? "Checking the operating-system integration."}</span>
        ...
        <CopilotSetup
          ...
          onClose={() => { setShowCopilotSetup(false); setInstallFlow(null); }}
            ...
            onInstallNpuRelease={(tag) => {
              // A release-sourced NPU model does not go through the planner:
              // this posts the install directly, and the bridge downloads,
              // verifies and unpacks the model + its runtime before serving.
              setShowCopilotSetup(false);
              void apiRequest(
                ...
                () => setAgentNotice("Installing the on-device model. This can take a few minutes; the Assistant will use it once it is ready."),
                (error) => setAgentNotice(error instanceof Error ? error.message : "The on-device model install could not be started."),
              );
            }}
      ...
      <ActionReviewDialog
        ...
        job={agentReview ? agentPlan?.proposed_job ?? null : pendingModelDownload ? { type: "model.download", payload: { inspection_job_id: pendingModelDownload.inspectionJobId, repo: pendingModelDownload.repo, file: pendingModelDownload.file, size_bytes: pendingModelDownload.sizeBytes } } : pendingModelDeploy?.result?.path ? { type: "model.deploy", payload: { path: pendingModelDeploy.result.path, port: 0, mode: modelRuntimeMode } } : null}
        onApprove={() => {
