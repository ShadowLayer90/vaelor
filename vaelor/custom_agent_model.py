...
# One home for the plan-shape rejection message so the parse fallback and the
# schema check raise the identical user-facing text without duplicating the
# literal (which the cross-module vocabulary guard would otherwise count twice).
_INVALID_CONNECTOR_PLAN = "The model returned an invalid connector-call plan."
...
execute until a human approves them. Return an empty calls list when unnecessary."""
    ...
    # temperature=0 above means a naive retry would reproduce the identical
    # output, so there is nothing to retry: an unparseable or off-schema plan is
    # an honest failure. We surface it as one clean, plan-shaped ValueError and
    # log a single concise WARNING (endpoint, model, a short content snippet) so
    # the next occurrence is diagnosable from one journal line rather than a
    # multi-frame traceback. It is never silently degraded to an empty plan.
    content = body["choices"][0]["message"]["content"]
    try:
        return _connector_calls_from_plan(parse_model_object(content))
    except ValueError as error:
        snippet = " ".join(str(content or "").split())[:200]
        LOGGER.warning(
            "custom agent connector plan unusable "
            "(endpoint=%s model=%r): %s; content=%r",
            connection.get("base_url", "?"), model, error, snippet,
        )
        raise ValueError(_INVALID_CONNECTOR_PLAN) from error


def _connector_calls_from_plan(parsed: Any) -> list[Dict[str, Any]]:
    """Validate a parsed ``{"calls":[...]}`` plan into a bounded call list.

    Raises ``ValueError`` on any shape violation; the caller turns that into a
    single user-facing message and warning. Kept separate so the parse and the
    schema check share one ``except ValueError`` at the call site.
    """
    if not isinstance(parsed, dict) or set(parsed) != {"calls"} or not isinstance(parsed["calls"], list):
        raise ValueError(_INVALID_CONNECTOR_PLAN)
    calls = []
