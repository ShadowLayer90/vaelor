...
#: in-appliance "Install" action and the installer's first-boot fetch agree byte
#: for byte). ``flm_npu_release.install`` reads this release index, downloads the
#: model parts, reassembles and verifies them against the entry's sha256, and
#: unpacks the model. Derived from the same ``VAELOR_REPO`` / ``VAELOR_RELEASE_TAG``
...
NPU_RELEASE_SOURCE = os.environ.get(
    ...
    "https://api.github.com/repos/{}/releases/tags/{}".format(
        NPU_RELEASE_REPO, NPU_RELEASE_TAG
    ),
)
...
NPU_RELEASE_MODELS: Tuple[Dict[str, Any], ...] = (
    _entry(
        ...
        # the real source is `source_url` (the GitHub release). engine="flm-npu" +
        # `release_tag` route the install through the bridge and serve it as that
        # flm tag. The sha256 is the REASSEMBLED model tar's digest - the split
        # `qwen35-4b-npu2.tar.part*` release assets concatenated - the same pin
        # `deploy/fetch-npu-model.sh` verifies, and the only trust anchor the
        # install honours (never the release JSON).
        "vaelor/Qwen3.5-4B-NPU2", "qwen35-4b-npu2.tar",
        3_597_588_480,
        "80401bba2fc26896a7cac8ef23629ee495b20b1b853bfb9ce5202562772ca0e4",
        SIZE_MEASURED,
...
def catalog_surface_by_file(file: str) -> str:
    ...
    The installed-model listing knows a file by name, not by repository (a
    deployed Compose file records only ``LLAMA_ARG_MODEL``), so this is the
    filename-keyed sibling of :func:`catalog_surface`. It keeps the same
    ambiguity rule as :func:`model_footprint.identify_by_file` (VD-065): a
    filename more than one entry claims - or none - resolves to ``""`` rather
    than guessing a surface, so a row is never labelled for the wrong tier and
    a deploy is never routed off a guess. The UI reads this to say whether "Use
    model" switches the AI-Chat model or the Assistant's, and to send the
    surface explicitly so a GPU chat model can never fall back to the Assistant.
    """
    if not file:
        return ""
    surfaces = {
    ...
    return surfaces.pop() if len(surfaces) == 1 else ""
