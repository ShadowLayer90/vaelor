...
#: The ROOT-ONLY retention of the wheel that produced the *current* install, so
...
#: with the retained wheel copy and its integrity record both ``0600``. The
#: staging area the executor DOES write (``STAGE_ROOT``) is a separate tree, so
#: the retained rollback bytes leave the workloads-writable area entirely and
#: cannot be tampered between recording and a rollback (#130/#178, closed on the
#: rollback path).
RETAINED_DIR = Path(
    env_value(
        "VAELOR_UPGRADE_RETAINED", "PM_UPGRADE_RETAINED",
        "/var/lib/vaelor/upgrade-retained",
    )
)
#: pip requires a PEP 427 wheel FILENAME (name-version-...-any.whl); a fixed
#: "current.whl"/"incoming.whl" is rejected with "Invalid wheel filename". So
#: every root-only copy keeps the staged wheel's OWN name, and the two copies -
#: the forward install source and the rollback-retained target - live in separate
#: root-only SUBDIRECTORIES of ``RETAINED_DIR`` so their identical names never
#: collide. A small JSON record pins the rollback target's path, digest and size.
#:
#: The forward copy closes a TOCTOU: the staged wheel is in a workloads-writable
#: dir, so verifying it in place and handing the SAME path to pip lets the
#: executor user swap the bytes between the hash and pip's read. The broker copies
#: the staged bytes into ``VERIFIED_SUBDIR`` (root:root) FIRST, verifies THAT
#: copy, and installs it - the bytes hashed are exactly the bytes installed.
VERIFIED_SUBDIR = "verified"
CURRENT_SUBDIR = "current"
RETAINED_RECORD_NAME = "record.json"
...
def read_current_wheel(directory: Path = RETAINED_DIR) -> Optional[Dict[str, Any]]:
    """The retained previous wheel and its pinned integrity metadata, if present.

    Returns ``{"wheel": Path, "sha256": str, "bytes": int}`` when the record
    parses, carries all three fields, and the retained wheel file still exists;
    otherwise ``None`` (a missing, malformed, or incomplete record, or a record
    whose wheel copy has gone). The record dict is what the rollback re-verifies
    the retained bytes against before reinstalling them as root.
    """
    try:
        record = json.loads(
            (Path(directory) / RETAINED_RECORD_NAME).read_text(encoding="utf-8")
        )
        wheel = Path(str(record["wheel"]))
        sha256 = str(record["sha256"])
        byte_length = int(record["bytes"])
    except (OSError, ValueError, KeyError, TypeError):
        return None
    if not wheel.is_file():
        return None
    return {"wheel": wheel, "sha256": sha256, "bytes": byte_length}


def record_current_wheel(
    version: str,
    wheel: Path,
    sha256: str,
    byte_length: int,
    ...
    The bytes are COPIED out of the workloads-writable staging area into
    ``directory`` (``root:root 0700``), which is what makes the rollback target
    tamper-proof: the retained copy leaves the area ``vaelor-workloads`` can
    write, so a later rollback reinstalls bytes the executor user can neither
    rewrite nor unlink. The record pins the copy's SHA-256 and byte length so the
    rollback re-verifies them at the privilege boundary, exactly as the forward
    path does for the staged wheel (#130/#178).
    """
    directory = Path(directory)
    directory.mkdir(parents=True, exist_ok=True)
    os.chmod(directory, 0o700)
    # Keep the wheel's real PEP 427 name (pip rejects a renamed wheel) in a fresh
    # root-only subdir, so the rollback reinstall gets a valid filename and stale
    # copies of a prior version do not linger.
    current_subdir = directory / CURRENT_SUBDIR
    shutil.rmtree(current_subdir, ignore_errors=True)
    current_subdir.mkdir(parents=True, exist_ok=True)
    os.chmod(current_subdir, 0o700)
    retained_wheel = current_subdir / Path(wheel).name
    shutil.copyfile(Path(wheel), retained_wheel)
    os.chmod(retained_wheel, 0o600)
    _atomic_write(
        directory / RETAINED_RECORD_NAME,
        {
            "version": str(version),
...
def _wheel_matches(wheel: Path, sha256: str, byte_length: int) -> bool:
    """True when ``wheel``'s byte length AND SHA-256 equal the pinned values.

    The single digest+size check shared by both privilege boundaries: the
    forward path re-verifies the staged wheel against the plan pin
    (:func:`_verify_staged_wheel`), and the rollback re-verifies the retained
    wheel against its recorded pin before reinstalling it as root. A missing file
    is a non-match, never an exception.
    """
    resolved = Path(wheel)
    if not resolved.is_file():
        return False
    if resolved.stat().st_size != int(byte_length):
        return False
    digest = hashlib.sha256()
    with resolved.open("rb") as handle:
        for chunk in iter(lambda: handle.read(4 * 1024 * 1024), b""):
            digest.update(chunk)
    return digest.hexdigest() == str(sha256)
...
def _verify_staged_wheel(
    ...
    stage_root: Path,
    *,
    verified_dir: Path = RETAINED_DIR,
) -> Path:
    """Copy the staged wheel into a root-only path, verify THAT copy, return it.

    #130/#178, pin enforced at the privilege boundary WITHOUT a TOCTOU: the
    executor that stages the plan runs unprivileged, and the staged wheel sits in
    a workloads-writable directory. Verifying the staged file and then handing the
    SAME path to pip reads it twice, so the executor user could swap the bytes
    between the hash and pip's read and get *root* to install arbitrary bytes into
    ``/opt/vaelor/venv``. So the broker: (1) refuses a wheel outside ``stage_root``
    (a plan can never aim the install at an arbitrary path); (2) COPIES the staged
    bytes into ``verified_dir`` - a ``root:root 0700`` directory the executor user
    can neither write nor unlink; (3) recomputes the SHA-256 and byte length
    against the plan pin on THAT copy; (4) returns the copy for pip to install.
    The bytes hashed are exactly the bytes installed, and once copied they are out
    of the executor's reach - so a swap during the copy fails verification and a
    swap after it is impossible.
    """
    ...
    # Keep the wheel's real PEP 427 name (pip rejects a renamed wheel) in a fresh
    # root-only subdir, cleared first so a prior version's copy never lingers.
    verified_subdir = verified_dir / VERIFIED_SUBDIR
    shutil.rmtree(verified_subdir, ignore_errors=True)
    verified_subdir.mkdir(parents=True, exist_ok=True)
    os.chmod(verified_subdir, 0o700)
    verified = verified_subdir / resolved.name
    shutil.copyfile(resolved, verified)
    os.chmod(verified, 0o600)
    if not _wheel_matches(
        verified, str(plan.get("sha256", "")), int(plan.get("bytes", -1))
    ):
        raise UpgradeFailure(
...
#: The systemd unit directory the shipped units are installed into.
SYSTEMD_DIR = Path("/etc/systemd/system")


def _bundled_units_dir() -> Path:
    """The ``appliance_units`` dir inside the INSTALLED ``vaelor`` package.

    Read from the on-disk package, not from the source tree, so that after a
    ``pip --force-reinstall`` (forward apply OR rollback) it reflects the units
    that shipped in the just-installed wheel - the whole point of refreshing the
    units beside the reinstall. ``importlib.resources`` finds the package where
    it is actually installed (``/opt/vaelor/venv/.../vaelor``); the
    ``__file__`` sibling is the fallback for an editable/source layout where the
    resources traversable is not a real filesystem path.
    """
    try:
        resource = importlib.resources.files("vaelor") / "appliance_units"
        path = Path(str(resource))
        if path.is_dir():
            return path
    except (ModuleNotFoundError, TypeError, OSError):
        pass
    return Path(__file__).parent / "appliance_units"
...
def _refresh_units(
    ...
    *,
    ...
    """Install every shipped ``vaelor-*.service`` that is missing or changed.

    The deferred half of #178 / VD-102: a release that changes a ``.service``
    (ExecStart, Environment, After/Requires, User ...) used to keep the OLD unit
    until an installer re-run, because the broker did ``pip install`` + restart
    and never touched ``/etc/systemd/system``. So for each unit the wheel ships
    (read from the ON-DISK installed package via :func:`_bundled_units_dir`, so
    a forward apply reflects the new wheel and a rollback the reinstalled
    previous wheel), compare bytes to the on-box copy and write it (mode 0644,
    root-owned) when it is missing or differs. If ANY unit changed, run
    ``systemctl daemon-reload`` so the restart that follows picks up the new
    units, and return True; if none changed, do nothing and return False.

    Only the shipped set is ever written - a unit is never deleted - and a write
    error propagates to the caller's try, rolling the apply back like any other
    failure at the privilege boundary. ``source_dir``/``systemd_dir`` are
    injectable so tests stay off the real ``/etc/systemd/system``.
    """
    source = Path(source_dir) if source_dir is not None else _bundled_units_dir()
    systemd_dir = Path(systemd_dir)
    changed = False
    for unit in sorted(source.glob("vaelor-*.service")):
        target = systemd_dir / unit.name
        shipped = unit.read_bytes()
        if not target.is_file() or target.read_bytes() != shipped:
            systemd_dir.mkdir(parents=True, exist_ok=True)
            target.write_bytes(shipped)
            os.chmod(target, 0o644)
            changed = True
    if changed:
        _run(runner, ["/usr/bin/systemctl", "daemon-reload"], timeout=60)
    return changed
...
def perform_upgrade_apply(
    ...
    *,
    ...
    snapshot_current: Callable[[], Optional[Dict[str, Any]]] = read_current_wheel,
    record_current: Callable[[str, Path, str, int], None] = record_current_wheel,
    ...
    try:
        ...
        # until an installer re-run (the deferred half of #178 / VD-102).
        _refresh_units(runner, systemd_dir=systemd_dir)
        ...
        # Retain the VERIFIED wheel and the plan's pinned digest/size, so the
        # rollback can re-verify the retained bytes at the privilege boundary
        # the way the forward path just did (#130/#178).
        record_current(to_version, verified, str(plan["sha256"]), int(plan["bytes"]))
        refresh(runner)
        outcome = {
            "ok": True,
            "completed": True,
        ...
        outcome = _rollback(
            ...
            health_gate, read_version, error, systemd_dir=systemd_dir,
        )
    finally:
...
def _rollback(
    ...
    *,
    ...
    ``from_version`` again. And, like the forward path, it re-verifies the bytes
    it is about to install as root: ``snapshot`` is the retained record
    (``{"wheel": Path, "sha256": str, "bytes": int}``), and the retained wheel's
    digest and length are recomputed against it BEFORE the reinstall. A mismatch
    is a safe refusal - the box stays on the failed new version rather than
    running unverified bytes as root (#130/#178, closed on the rollback path).
    """
    detail = str(error)[:500]
    wheel = Path(snapshot["wheel"]) if snapshot else None
    ...
    if not _wheel_matches(wheel, str(snapshot["sha256"]), int(snapshot["bytes"])):
        return {
            "ok": False,
            "completed": False,
            "from_version": from_version,
            ...
            "error": detail,
            "rollback_error": (
                "The retained previous wheel failed its integrity check; "
                "refusing to reinstall unverified bytes as root."
            ),
            "completed_at": int(time.time()),
        }
    try:
        _pip_reinstall(runner, wheel)
        # Restore the units that match the reinstalled (previous) wheel before
        # the restart, the same order the forward path uses (#178 / VD-102).
        _refresh_units(runner, systemd_dir=systemd_dir)
        _restart_units(runner)
        health_gate()
