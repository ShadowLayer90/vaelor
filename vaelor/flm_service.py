...
(a validated ``flm serve`` invocation) comes from here. Vaelor launches the
binary **directly** (VD-001), not through ``lemond``; the Lemonade snap is kept
only because the binary lives in its cache (VD-013).
...
#: The ``flm-real`` binary. A fixed path, never a ``PATH`` lookup: the launch
#: crosses a root boundary and the executable must not be anything a caller or a
#: mutable environment could redirect. Read off the Z2 (VD-001 FLM facts).
FLM_BINARY = "/var/snap/lemonade-server/common/cache/lemonade/bin/flm/npu/flm-real"

#: ``HOME`` for the launched process. FLM resolves its model cache relative to
#: this, and the binary lives under the Lemonade snap's common directory, so a
#: launch with the wrong ``HOME`` finds no models (VD-001).
FLM_HOME = "/var/snap/lemonade-server/common"
...
def flm_serve_env() -> Dict[str, str]:
    """The launch environment: the root service's own, with ``HOME`` pinned.

    ``HOME`` is the one variable this must set — FLM resolves its model cache
    relative to it, and the binary lives under the Lemonade snap's common
    directory (VD-001). Everything else is inherited from the privileged
    service's environment rather than stripped: flm-real is a complex NPU binary
    that may need the snap's own runtime paths (its libraries, XDG dirs) to
    start, and a minimal hand-built environment is exactly the kind of plausible
    mechanism CLAUDE.md warns is refuted the moment it is run on the box. The
    environment is not caller-controlled — it is the root unit's own — so the
    security boundary is the validated argv above, not this dict.
    """
    return {**os.environ, "HOME": FLM_HOME}
...
def _sandbox_visible_binary_marker() -> bool:
    """The signal the sandbox leaves VISIBLE that the flm binary set is installed.

    ``FLM_BINARY`` sits two levels below ``.../bin`` (``.../bin/flm/npu/flm-real``).
    The Aug-24 refresh locked ``.../bin/flm`` and ``.../bin/flm/npu`` to
    ``0770 root:root``, but ``.../bin`` itself stays world-traversable, so the
    executor can STAT the ``.../bin/flm`` entry from it even though it cannot
    traverse INTO it to reach the leaf (stat-ing an entry needs traverse on its
    parent, not on the entry). ``.../bin/flm`` is the flm-family directory the
    snap creates only when the binary set is installed — a fresh box or a Pi has
    none of this tree — so its presence is the sandbox-visible "flm is installed"
    signal, the same move ``device_present`` made to the sysfs class node. It is
    a directory-level signal (not the leaf itself), which is why the truthful
    bridge probe is preferred over it whenever the bridge can answer.
    """
    return _path_present(str(Path(FLM_BINARY).parent.parent))
...
def discover_npu_serving(
    ...
    *,
    ...
    binary and device without touching the filesystem. Model presence rests on
    the installed inventory (:data:`INSTALLED_FLM_MODELS`, read off the measured
    machine) rather than a live cache scan: the two filesystem checks below are
    the live half of the gate, and they are what make a Pi honestly absent. A
    deeper cache probe is a noted follow-up, not a fabricated path.
    ...
    **Binary presence is read where it is true, not off a root-only path.** For
    ...
    return npu_serving_verdict(
        ...
        model_installed=str(tag or "") in set(installed),
        tag=tag,
    )
