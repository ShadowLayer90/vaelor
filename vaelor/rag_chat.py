...
#: The only keys a stored `performance` summary may carry, each a bounded
#: float, so a reload restores exactly the compact timing line the answer was
#: shown with and nothing a caller happened to attach alongside it.
_PERFORMANCE_KEYS = ("total_seconds", "ttft_seconds", "prefill_tps", "decode_tps")


def _performance_view(value: Any) -> Dict[str, float]:
    """A bounded float-only view of a per-answer timing summary."""
...
class RagChatStore:
    ...
    def ingest(
        ...
        # `raw` is the uploaded FILE bytes for a binary format (PDF, Office);
        # `content` is already-extracted text (the frontend sends plain-text
        # files that way). Either becomes `text`, which is what gets chunked.
        name = str(name).strip()[:180]
        if not name:
            raise RagChatError("A document name is required.")
        # Confirm the caller owns the collection BEFORE the potentially
        # expensive extraction, so a bogus collection id cannot make the server
        # parse a large upload for nothing (the insert below re-checks it).
        with closing(self._connect()) as connection:
            self._owned_collection(connection, actor, collection_id)
        limit_mib = MAX_DOCUMENT_BYTES // (1024 * 1024)
        if raw is not None:
            if len(raw) > MAX_DOCUMENT_BYTES:
                raise RagChatError(f"The document exceeds the {limit_mib} MiB limit.")
            try:
                text = extract_text(raw, media_type, name)
            except DocumentExtractionError as error:
                raise RagChatError(str(error)) from error
            size_bytes = len(raw)
            ...
            if not encoded or len(encoded) > MAX_DOCUMENT_BYTES:
                raise RagChatError(
                    f"Document text is required; maximum size is {limit_mib} MiB.")
            ...
            with closing(self._connect()) as connection:
                ...
                connection.execute(
                    ...
                    (document_id, collection_id, actor, name, str(media_type)[:100],
                     digest, size_bytes, len(parts), now),
                )
    ...
    def retrieve(
        ...
        limit: int = 5,
        lead_on_empty: bool = False,
    ):
        terms = re.findall(r"[A-Za-z0-9]{2,}", str(query))[:20]
        # A query with no usable terms can still want the document: "summarize
        # it" is all stop-length words. With `lead_on_empty` the caller asks for
        # the document's opening even then, so an empty term list is not an early
        # exit unless nothing was selected to lead from.
        if (not terms and not lead_on_empty) or not collection_ids:
            return []
        ...
        with closing(self._connect()) as connection:
            ...
            if terms:
                ...
                rows = connection.execute(
                    ...
                    """,
                    (expression, *selected, capped),
                ).fetchall()
            # **Keyword retrieval found nothing, but a document IS selected.**
            # FTS matches the literal question, so a document-directed request
            # that shares no words with the document ("summarize this",
            # "what does it say") returns zero chunks - and the model, handed a
            # prompt with no document in it, truthfully answers that it sees
            # none. When the caller opted in, fall back to the leading chunks of
            # each selected document so a whole-document ask has the text to work
            # from rather than depending on lexical overlap. Ordered by document
            # then ordinal so each document contributes its opening.
            if not rows and lead_on_empty:
                rows = connection.execute(
                    f"""
                    SELECT ch.id,ch.ordinal,ch.content,d.id AS document_id,
                           d.name AS document_name,c.id AS collection_id,
                           c.name AS collection_name,0.0 AS score
                    FROM rag_chunks ch
                    JOIN rag_documents d ON d.id=ch.document_id
                    JOIN rag_collections c ON c.id=ch.collection_id
                    WHERE ch.collection_id IN ({placeholders})
                    ORDER BY ch.document_id, ch.ordinal LIMIT ?
                    """,
                    (*selected, capped),
                ).fetchall()
        return [dict(row) for row in rows]
