    ...
    def run_once(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if job["type"] == "host.vnc.enable":
                ...
                return self.store.finish(job["id"], state="completed",
                    message="Optional host browser desktop is ready", result=result)
            if job["type"] == "host.docker.install":
                ...
                return self.store.finish(job["id"], state="completed",
                    message="Docker and Compose installed; Vaelor access is being refreshed", result=result)
            if job["type"] == "host.docker.repair":
                if job["payload"].get("confirm") != "repair-docker":
                    raise ValueError("Confirm that Docker should be repaired on this host.")
                self._checkpoint(20, "Restarting the Docker runtime to rebuild its storage", "starting")
                result = self.host_desktop.repair_docker()
                return self.store.finish(job["id"], state="completed",
                    message="Docker was restarted and its storage area rebuilt", result=result)
