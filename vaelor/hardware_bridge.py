...
class _HardwareRuntime:
    ...
    def flm_install_release(self, source_url: str, expected_sha256: str) -> dict[str, Any]:
        ...
        streamed to disk); this targets the Vaelor-owned FastFlowLM directory and
        the ``flm-real`` binary directory (`flm_service.FLM_MODEL_PATH` /
        `FLM_BINARY`), so once the model AND its bundled runtime land there the
        existing supervisor serves the installed tag with no other change - the
        turnkey equivalent of the manual place-model-and-swap-runtime step, done
        as root behind the bridge.
        """
        from pathlib import Path

        from . import flm_npu_release
        from .flm_service import FLM_BINARY, FLM_MODEL_PATH
