...
#: A request to work from the WHOLE selected document rather than to find a
#: passage in it.
#:
#: Retrieval is keyword FTS over the literal message, so "summarize it" - all
#: stop-length words that appear in no document - matches nothing, and the model,
#: handed a prompt with no document, reports it sees none. `retrieve` can fall
#: back to the document's leading chunks (`lead_on_empty`), but only when the ask
#: is document-directed: a genuine keyword search that matched nothing
#: ("zzzznomatch") must still honestly report zero passages rather than have a
#: document injected under it. This predicate draws that line - a summary verb, a
#: reference to "the document/file/attachment", or "what does it/this say" - and
#: is read only where keyword retrieval already came back empty, so a false
#: positive costs at most the document's opening on a question that found nothing
#: anyway, never a wrong passage over a question that did.
_WHOLE_DOCUMENT_REQUEST = re.compile(
    r"\b(?:summar(?:y|ise|ize|ising|izing|ised|ized)|tl;?dr|recap|gist|"
    r"synopsis|synopses|overview|paraphrase|outline)\b"
    r"|\b(?:the|this|that|these|those|my|attached|uploaded)\s+"
    r"(?:document|documents|doc|docs|file|files|attachment|attachments|"
    r"pdf|paper|report|text|note|notes|contents?)\b"
    r"|\bwhat\s+does\s+(?:it|this|that)\s+say\b",
    re.IGNORECASE,
)


def wants_whole_document(message: Any) -> bool:
    """Whether ``message`` asks to work from the whole selected document.

    Used only to decide `RagChatStore.retrieve(lead_on_empty=...)` when keyword
    retrieval returned nothing, so the send and regenerate routes cannot drift on
    whether "summarize this" gets the document. See `_WHOLE_DOCUMENT_REQUEST`.
    """
    return bool(_WHOLE_DOCUMENT_REQUEST.search(str(message or "")))
