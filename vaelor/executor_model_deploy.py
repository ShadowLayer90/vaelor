...
class ExecutorModelDeployMixin:
    ...
    def _deploy_model(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # below (no such device). An ``ai-chat`` surface is exempt
        # (`should_serve_on_npu` returns False for any non-assistant surface), so a
        # GPU chat GGUF is never re-served on the NPU.
        ...
        # A non-FP4 ai-chat model serves on the GPU fork when this machine can
        # (gfx1151 + fork + runtime); otherwise, or on a fork serve failure
        # (FIX 1), it falls through to the compose path below.
        if serve_as_chat and not gpu_fork_fallback_reason and self._gpu_rocm_chat_available():
            fork, gpu_fork_fallback_reason = self._gpu_fork_or_fallback(
                payload, model or self._model_file(path)
            )
            if fork is not None:
                return fork
                ...
                with urllib.request.urlopen(health_url, timeout=3) as response:
                    if response.status == 200:
                        ...
                        return {
                            ...
                            "credential_id": credential["id"],
                            # FIX 1: note a fall-back from a failed GPU-fork serve
                            # (empty reason when the fork was never attempted).
                            "gpu_fork_fallback": bool(gpu_fork_fallback_reason),
                            "gpu_fork_fallback_reason": gpu_fork_fallback_reason,
                        }
