...
def with_performance(result: Any, body: Any) -> Any:
    """Carry a normalized per-answer timing summary from a raw model body.

    The connected path parses the answer out of the body and discards the rest,
    but the body also holds the wall-clock and usage timings the client attached
    (see ``inference_client.PERFORMANCE_KEY``). Copying it onto the answer dict
    is what lets the Assistant show the same compact performance line AI Chat
    does; an absent or empty summary attaches nothing.
    """
    if isinstance(result, dict) and isinstance(body, dict) and body.get("performance"):
        result["performance"] = body["performance"]
    return result
