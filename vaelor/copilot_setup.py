...
def recommend_local_models(hardware: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # This base recommendation and its `catalog` list are the shared, installable
    # GGUF pool (CPU/GPU, on the RAM ladder). The on-device NPU model is
    # deliberately NOT part of either: it runs on the neural processor rather than
    # system RAM, installs through its own release path, and is surfaced solely by
    # `_npu_assistant_recommendation`, which `copilot_setup_status` merges over this
    # base as `recommendation` when the NPU is actually serving. Making the NPU
    # model the base primary here was two defects: it put the model in the GGUF
    # catalog list (a "Review installation" card in the GPU model list, where the
    # two pools are not shared), and — because the catalog path inherits this base
    # when no GPU fork is provisioned — left the catalog screen recommending a
    # model absent from its own grid. The NPU pick lives only in the override now.
...
def _npu_assistant_recommendation(hardware: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Show the card whenever the NPU is CAPABLE of serving - the flm-real binary
    # and the accelerator device are present - not only once the model is on
    # disk. That is what makes the card the install entry point: on a box that
    # has the runtime but not yet the model it offers the ~3.4 GB download; once
    # the model is present it shows "no download". Gating on `available` (which,
    # correctly, requires the model on disk for the serving consumers) would hide
    # the only UI path to install the model.
    if not (capability.get("binary_present") and capability.get("device_present")):
        return {}
    model_on_disk = bool(capability.get("model_installed"))
    ...
    # Present tense only when the model is actually on disk. With the model
    # absent the card is the install entry point, so it must speak in the future
    # ("will hold" / "will run") and must not contradict the Download row below.
    if not resident:
        memory_note = (
        ...
        memory_note = (
            ...
            "measured on this hardware.".format(resident / GIB)
        )
    else:
        memory_note = (
            "Will hold about {:.1f} GiB of neural-processor memory while "
            "serving, measured on this hardware.".format(resident / GIB)
        )
    experience = (
        "Runs the Assistant on the neural processor (XDNA2 NPU) via flm-real, "
        "chosen on reasoning accuracy."
        if model_on_disk else
        "Will run the Assistant on the neural processor (XDNA2 NPU) via "
        "flm-real once installed, chosen for reasoning accuracy."
    )
    # Honest download copy from LIVE disk state: on a box that already has the
    # model there is nothing to fetch; on one that does not, setting it up
    # downloads the release artifact first. The size is read from the catalog
    # release entry so it cannot drift from what is actually fetched. Sized in
    # GiB (binary), matching the executor checkpoint and the catalog so a user
    # sees the same figure everywhere.
    releases = npu_release_models()
    release_bytes = int(releases[0].get("download_bytes") or 0) if releases else 0
    if model_on_disk:
        download_bytes = 0
        size_note = "Served on the neural processor; no download."
    else:
        download_bytes = release_bytes
        size_note = (
            "Downloads the on-device model (about {:.1f} GiB) the first time you "
            "set it up, then serves it on the neural processor.".format(
                release_bytes / GIB)
            if release_bytes else
            "Downloads the on-device model the first time you set it up, then "
            "serves it on the neural processor.")
    ...
    primary = {
        ...
        "backend": "flm-real",
        "device": "npu",
        # True only when the model is actually on disk: the card is shown while
        # CAPABLE (binary+device) so it can be the install entry point, but it
        # must not claim present-tense NPU serving before the model exists.
        "served_on_npu": model_on_disk,
        "flm_tag": flm_tag,
    }
    # Route the card's button to the NPU release install, not the GGUF planner.
    # Without `engine`/`release_tag` the frontend cannot tell this is the
    # release-installed on-device model, so its button fell through to
    # `onChooseLocal` - which opened the deployment assistant instead of
    # installing/serving the model. This is always the on-device NPU model
    # (installed and served through the release path, whether or not it is on disk
    # yet), so set the markers unconditionally rather than gating on a catalog
    # lookup: even in the latent case of a served tag with no catalog release,
    # routing to the install surfaces a clear job error in the status view,
    # whereas the planner fallback fails with a confusing "choose a downloaded
    # GGUF" that cannot apply to an NPU model (review finding 8).
    if flm_tag:
        primary["engine"] = "flm-npu"
        primary["release_tag"] = flm_tag
    return {
        "tier": "accelerated",
        ...
        "can_install": True,
        "storage_reason": "",
        # Present tense only once the model is on disk. With it absent the
        # Download row says "Downloads ... GiB", so the rationale must not claim
        # "no model is downloaded"; it speaks to what happens once installed.
        "rationale": (
            ...
            if model_on_disk else
            "Its fine-tuned on-device model runs on this machine's neural "
            "processor once installed - the fastest, most private option; "
            "setting it up downloads the model first."
        ),
        # So a reader can tell an NPU-served recommendation from a GGUF one. True
        # only when the model is on disk, matching the primary above.
        "served_on_npu": model_on_disk,
