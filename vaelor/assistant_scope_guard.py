...
def misrouted_refusal(question: Any, answer_text: Any, source: str = "") -> bool:
    ...
    **VD-121: the guard and the redirect must read one out-of-scope
    determination.** This guard withholds an answer that hands an APPLIANCE
    question to AI Chat, and it keyed "appliance" on `is_appliance_question`
    alone. But VD-121 sends a definitional question that names hardware ("what is
    a CPU?") to AI Chat on purpose - `is_appliance_question` is True for it
    (``cpu``), yet the redirect is CORRECT, not misrouted. Keying only on
    `is_appliance_question`, the guard replaced the Assistant's own legitimate
    redirect with the withhold notice ("that question is about this appliance…
    name the reading you want"), which a live test caught. Deferring to
    `is_out_of_scope_question` - the very predicate `knowledge_redirect` consults
    to issue the redirect - makes the two agree: a redirect the Assistant issued
    for an out-of-scope question is never second-guessed here, while a reading or
    diagnostic question the model wrongly deflects ("what is my CPU temperature?"
    → "ask AI Chat") is still caught, because it is not out of scope.
    """
    if str(source or "") in SAFETY_SOURCES:
        return False
    if not is_appliance_question(question):
        return False
    # Imported lazily, not at module load: a module-level edge from this module
    # to `assistant_intents` shifts the order in which the reachability
    # instrumentation sees the intent tables and trips its self-certified ratchet
    # in the full-suite import order (test_vocabulary_reachability). The predicate
    # is only needed at answer time, and `assistant_intents` is already loaded by
    # then, so the deferral costs nothing - the same pattern `select_tools` and
    # `identify_by_file` use to avoid a load-time dependency.
    from .assistant_intents import is_out_of_scope_question
