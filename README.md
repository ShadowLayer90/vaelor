...
On the target box (Debian-family arm64 / amd64):

```bash
git clone https://github.com/ShadowLayer90/vaelor.git ~/vaelor

# One command. The installer downloads the wheel from the release itself (no wheel
# path to find, no cd into the clone) and, on a box with a neural accelerator, also
# fetches the on-device NPU assistant model (~3.4 GB, split on the release):
...
To skip the large NPU model download and defer it, install with `--without-npu-model` and
fetch it later:

```bash
sudo ~/vaelor/deploy/fetch-npu-model.sh
```
